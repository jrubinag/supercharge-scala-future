...
object SearchResult {
  ...
  def apply(flights: List[Flight]): SearchResult = {
    val withCheapestOnDuplicated = flights.groupBy(_.flightId).map{ case (id,flights) =>
      id -> new SearchResult(flights).cheapest
    }.values.flatten

    new SearchResult(withCheapestOnDuplicated.toList.sorted(bestOrdering))
  }
