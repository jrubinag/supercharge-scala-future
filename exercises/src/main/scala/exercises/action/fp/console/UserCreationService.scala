...
class UserCreationService(console: Console, clock: Clock) {
  ...
  val readName: IO[String] = {
    val a1 = writeLine("What's your name?")
    val a2 = readLine
    a1.andThen(a2)
  }
  ...
  val readDateOfBirth: IO[LocalDate] = (for {
    _ <- writeLine("What's your date of birth? [dd-mm-yyyy]")
    line <- readLine
    dateOfBirth <- parseDateOfBirth(line)
  } yield dateOfBirth).onError(_ => writeLine("""Incorrect format, for example enter "18-03-2001" for 18th of March 2001"""))
  ...
  val readSubscribeToMailingList: IO[Boolean] =
    (for {
     ...
     line <-  readLine
      lineAsBoolean <- parseLineToBoolean(line)
    } yield lineAsBoolean).onError(_ =>  writeLine("""Incorrect format, enter "Y" for Yes or "N" for "No""""))
