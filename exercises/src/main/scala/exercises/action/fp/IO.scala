...
trait IO[A] {
  ...
  def andThen[Other](other: IO[Other]): IO[Other] = this.flatMap(_ => other)
...
//  }
  ...
  def map[Next](callBack: A => Next): IO[Next] = flatMap(value  => IO(callBack(value)))
...
//  def retry(maxAttempt: Int): IO[A] =
//    for {
//      _ <- IO(require(maxAttempt > 0, "maxAttempt must be greater than 0"))
//      attemptRest <- attempt
//      res <- attemptRest  match {
//        case Success(value) => IO(value)
//        case Failure(exception) =>
//          if(maxAttempt == 1) IO.fail(exception)
//          else retry(maxAttempt - 1)
//      }
//    } yield res
...
object IO {
  ...
  //with performance optimization for prepend
    actions.foldLeft(IO(List.empty[A])){ case (accIO, action) =>
      for {
        acc <- accIO
        resultFromAction <- action
      } yield resultFromAction :: acc
    }.map(_.reverse)
...
//        acc <- accIO
//        resultFromAction <- action
//      } yield acc ++ List(resultFromAction)
//    }
